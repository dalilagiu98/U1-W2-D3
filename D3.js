...
for(i = 0; i < starWarsCharacters.length; i++) {
  let valueName = starWarsCharacters[i].name
  charactersNames.push(valueName)
}
...
for( let i = 0; i < femaleCharacters.length; i++) {
  ...
  for( let j = 0; j < charactersNames.length; j++) {
    if (femaleName === charactersNames[j]){
      charactersNames.splice(j, 1)
    }
  }
}
console.log("new characters name are", charactersNames)

/* --EXTRA-- ESERCIZIO 10
  Crea una funzionalità che selezioni un elemento casuale dall'array "starWarsCharacters" e ne stampi in console le proprietà in modo discorsivo (a tuo piacimento).
*/
const randomElement = Math.floor(Math.random() * starWarsCharacters.length);
const randomCharacters = starWarsCharacters[randomElement]
console.log("You've choose randomly this Star Wars Character:")
console.log("Name:", randomCharacters.name)
